					...
					case 'edititem':
						...
						**/
							...
							if ($main_submit_permission) {
								...
								$term = null;
								if(isset($_GET['ua']) && isset($_GET['pfupload_listingtypes']) && 'newitem' == $_GET['ua']) {
									$button_style = 'display';
									$term = get_term($_GET['pfupload_listingtypes']);
								}

								//jschen added, get term by the url of last page
								$url = $_GET['url'];
								if ($_GET['ua'] == 'newitem' && $url != '') {
										$pieces = explode("/", $url);
										$find = false;
										$cat = "";
										...
										if ($find) {
											global $wpdb;
											$result =	$wpdb->get_row( $wpdb->prepare( "select term_id from $wpdb->terms where slug=%s", $cat));
								...
								if($term) {
										$item_defaultvalue_output = $term->term_id;
										$this->FieldOutput .= '<div style="padding: 10px 10px 0">'. $term->name . '</div>';
	                  ...
	                  $this->ScriptOutput .= '$.pf_submit_page_map();';
								...
								if(isset($_GET['pfupload_listingtypes'])) {
									...
									foreach ($listingtype_values as $listingtype_value) {
										...
										if ($multiple_select == 1) {
											$multiplesarray .= $listingtype_value->term_id . ',';
										}
										if ($subcat_select == 1) {
											$subcatsarray .= $listingtype_value->term_id . ',';
										}
									...
									$this->ScriptOutput .= $subcatsarray . $multiplesarray;
										...
										**/
								if($item_defaultvalue_output) {
									$this->FieldOutput .= '<div class="pf-category-container">';
								}else {
									$this->FieldOutput .= '<div class="pf-excludecategory-container">';
								}

											$this->FieldOutput .= '<div class="pfsubmit-title">'.esc_html__("信息",'pointfindert2d').'</div>';
											$this->FieldOutput .= '<section class="pfsubmit-inner">';
												...
												**/
													...
													<section class="pfsubmit-inner-sub">
								                        <label for="item_title" class="lbl-text">'.esc_html__('標題','pointfindert2d').':</label>
								                        <label class="lbl-ui">
												...
												**/
													...
													$this->FieldOutput .= '
													<section class="pfsubmit-inner-sub">
								                        <label for="item_desc" class="lbl-text">'.esc_html__('描述','pointfindert2d').':</label>
								                        <label class="lbl-ui">';
                                    ...
                                    if ($st4_sp_med == 1) {
										...
										**/
                                            ...
                                            if ($setup4_submitpage_imageupload == 1) {
                                                ...
                                                $this->FieldOutput .= '<div class="pfsubmit-title pfsubmit-inner-sub-image">' . esc_html__('圖片上傳', 'pointfindert2d') . '</div>';
                                                ...
                                                if (strpos($_SERVER['HTTP_USER_AGENT'], 'MSIE 9') !== false || $setup4_submitpage_status_old == 1) {
                                                    ...
                                                    if ($setup4_submitpage_imageupload == 1) {
                                                        ...
                                                        $this->FieldOutput .= ' 
																		dictDefaultMessage: "' . esc_html__('將文件拖放到這裡上傳!', 'pointfindert2d') . '<br/>' . esc_html__('你最多可上傳', 'pointfindert2d') . ' <div class=\'pfuploaddrzonenum\'>{0}</div> ' . esc_html__('張圖', 'pointfindert2d') . ' ' . sprintf(esc_html__('(每張圖最大: %dMB )', 'pointfindert2d'), $setup4_submitpage_imagesizelimit) . ' ".format($.drzoneuploadlimit),
        ...
        getOrientation(origFile, function(orientation) {
                ...
                reader.readAsDataURL(origFile);
                                                        ...
                                                        $this->FieldOutput .= ' 	
															...
															<a id="pf-ajax-fileuploadformopen" class="button pfmyitempagebuttonsex" style="width:100%"><i class="pfadmicon-glyph-512"></i> ' . esc_html__('點選上傳圖片', 'pointfindert2d') . '</a>
															';
                                                        $this->FieldOutput .= '</section>';
                                                    }
                                                    /**
                                ...
                                if   ($params['current_user']<>get_anonymous_user()->ID) {
										...
										**/
											if ($setup4_membersettings_paymentsystem == 1) {
												...
												$this->FieldOutput .= '<section class="pfsubmit-inner-sub">';
													...
													$this->FieldOutput .= '<div class="pfsubmit-inner-totalcost-output"><input name="pf_lpacks_payment_selection" id="pfm-payment-free" value="free" type="hidden"></div>';
				...
				if($params['formtype'] != 'errorview' && $params['formtype'] != 'banktransfer'){
					if($params['formtype'] != 'myitems' && $params['formtype'] != 'favorites' && $params['formtype'] != 'reviews' && $params['formtype'] != 'invoices' && $params['dontshowpage'] != 1 && $main_package_expire_problem != true){
			            $this->FieldOutput .='    
			                ...
			                if (!$hide_button) {
			                	$this->FieldOutput .= '
				                   <input type="submit" value="'.$buttontext.'" id="'.$buttonid.'" class="button blue pfmyitempagebuttonsex '. $button_style .'" data-edit="'.$params['post_id'].'"  />
			                   ';
			                }
