...
function pf_ajax_itemsystem(){
  ...
  $pfreturn_url = $setup4_membersettings_dashboard_link.$pfmenu_perout.'ua=myitems';
    if   ($user_id == get_anonymous_user()->ID) {
        $listingtype_id = $vars['pfupload_listingtypes'];
        $listingtype_term = get_term($listingtype_id, 'pointfinderltypes');
        $listingtype_slug = $listingtype_term->slug;
        $pfreturn_url = site_url('listings/' . $listingtype_slug);
    }
  $output_html = '';
